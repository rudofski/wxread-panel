...
> v0.1.1 变更：移除书城搜索与 Cloudflare Worker 代理；认证改为纯 PAT；热力图改自绘实现；Secrets 加密修复为 libsodium sealed box
...
| 8 | 技术栈 | **Vue 3 + Vite + TypeScript + Octokit + tweetnacl** |
...
| Secrets 加密 | tweetnacl + tweetnacl-util | libsodium `crypto_box_seal`（GitHub 要求的 curve25519 密封盒） |
...
│   ├── utils/sealedBox.test.ts  # 密封盒 roundtrip
...
### 5.4 curl_bash 获取（替代 v0.1.0 书城搜索）
...
| 层 | 范围 | 工具 | 数量 |
|----|------|------|------|
| 纯函数单测 | URL/错误解析、书库序列化（已随书城移除）、时间校验 | Vitest | — |
| Mock API 交互 | detectRepo / updateVariable / dispatch / 适配层 / 日志拉取 | Vitest + vi.mock | 7 |
| Store 测试 | auth 登录态、settings 配置状态 | Vitest + Pinia | 9 |
| 加密测试 | sealed box roundtrip | Vitest | 3 |
| E2E 冒烟 | 登录跳转 / token 输入 / 无 OAuth 按钮 | Playwright | 4 |

组件层未单独引入测试框架（`@vue/test-utils` 未使用），组件行为由 E2E 冒烟覆盖。

### 9.2 关键测试用例

- 适配层：新旧变量名兼容、分钟↔次数转换、默认值回退
- 错误解析：Cookie 过期、推送失败、网络超时 → 对应中文提示
- 加密：密封盒可用接收方私钥解封（roundtrip）、密文随机性
